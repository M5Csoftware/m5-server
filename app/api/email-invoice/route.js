...
function imageToBase64(imagePath) {
  try {
    ...
    return ext === '.svg' 
      ? "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNTAiIGhlaWdodD0iNTAiIHZpZXdCb3g9IjAgMCA1MCA1MCIgZmlsbD0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iNTAiIGhlaWdodD0iNTAiIGZpbGw9IiNlNWU3ZWIiLz48L3N2Zz4="
...
// Generate HTML for Invoice (matching client-side design EXACTLY)
function generateInvoiceHTML(invoiceData, qrCodeDataUrl) {
  const {
    customer = {},
    invoiceSummary = {},
    shipments = [],
  ...
  const totals = {
    ...
    weight: shipments?.reduce((sum, s) => sum + Number(s.totalActualWt || s.weight || 0), 0) || 0,
  ...
  return `
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .invoice-container { width: 210mm; min-height: 297mm; padding: 24px; background: #f9fafb; }
        ...
        .header-table th { text-align: left; border: 1px solid #000; padding: 6px 6px 12px 6px; background: #f2f2f2; width: 35%; }
        .header-table td { border: 1px solid #000; padding: 6px 6px 12px 6px; }
        .header-table tr:nth-child(2) th, .header-table tr:nth-child(2) td { padding-bottom: 16px; }
        ...
        /* Tables */
        table.invoice-table { width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 8px; }
        table.invoice-table th, table.invoice-table td { border: 1px solid #000; padding: 8px 8px 16px 8px; text-align: center; }
        table.invoice-table th { background: #e5e7eb; font-weight: bold; }
        table.invoice-table td.no-border-bottom { border-bottom: none; }
        table.invoice-table td.no-border-top { border-top: none; }
        ...
        /* QR Section */
        .qr-section { display: flex; justify-content: space-between; align-items: center; padding: 12px 8px; gap: 16px; }
        .qr-info { flex: 1; font-size: 11px; line-height: 2; }
        .qr-box { border: 1px solid #374151; padding: 1px; border-radius: 2px; }
        .qr-spacer { width: 150px; height: 100px; }
        
        /* Bank Details */
        .bank-section { display: flex; gap: 16px; justify-content: space-between; margin-bottom: 8px; }
    ...
    <body>
      <div class="invoice-container">
        ...
        <div class="header">
          ...
          <div class="header-info">
            <table class="header-table">
              ...
              <tr>
                <th>Date:</th>
                <td>${invoiceDate ? new Date(invoiceDate).toLocaleDateString("en-IN") : "DD/MM/YYYY"}</td>
              </tr>
            </table>
          </div>
        </div>
        ...
        <div class="grid-2">
          <div class="box">
            <div class="box-header">Bill To: ${customer?.name || "Customer Name"}</div>
            <div class="box-content">
              ...
              ${customer?.gstNo ? `<div><strong>GST:</strong> ${customer.gstNo}</div>` : ""}
              ${customer?.panNo ? `<div><strong>PAN No:</strong> ${customer.panNo}</div>` : ""}
              ${customer?.state ? `<div><strong>State:</strong> ${customer.state}</div>` : ""}
        ...
        <!-- Summary Table -->
        <h2 style="text-align: center; font-weight: bold; margin: 16px 0;">Invoice Summary</h2>
        <table class="invoice-table">
          ...
          <tbody>
            <tr>
              ...
              <td>${(totals.amount - totals.discount).toFixed(2)}</td>
        ...
        <div class="amount-words">
          ...
          <div>Round Off: <span class="value">${totals.roundOff.toFixed(2)}</span></div>
        </div>

        <!-- QR Code Section -->
        <div class="qr-section">
          <div class="qr-info">
            <div><strong>IRN:</strong> ${qr?.irnNumber || "N/A"}</div>
            <div><strong>ACK NO:</strong> ${qr?.ackNo || "N/A"}</div>
            <div><strong>ACK DATE:</strong> ${qr?.ackDate || "N/A"}</div>
          </div>
          <div class="qr-box">
            ${qrCodeDataUrl ? `<img src="${qrCodeDataUrl}" width="140" height="140" alt="QR Code" />` : ""}
          </div>
        ...
        <div class="bank-section">
          ...
          <div class="summary-box">
            <div class="summary-row"><span>Non Taxable Amount:</span><span>${totals.nonTaxable.toFixed(2)}</span></div>
            <div class="summary-row"><span>Taxable:</span><span>${(totals.amount - totals.discount).toFixed(2)}</span></div>
            ...
            <div class="summary-row"><span>Misc Charges:</span><span>${totals.miscChg.toFixed(2)}</span></div>
            <div class="summary-row"><span>Fuel Charges:</span><span>${totals.fuelChg.toFixed(2)}</span></div>
            <div class="summary-row"><span>Rounded Off:</span><span>${totals.roundOff.toFixed(2)}</span></div>
          </div>
        </div>
...
// Generate Shipment Details HTML (Page 2)
function generateShipmentDetailsHTML(shipments) {
  ...
  const rows = shipments.map((s) => {
    ...
    const weight = Number(s.totalActualWt || s.weight || 0).toFixed(2);
    const basicAmt = Number(s.basicAmt || s.amount || 0).toFixed(2);
    const discount = Number(s.discountAmt || s.discount || 0).toFixed(2);
    const taxable = Number(s.taxableAmount || (s.basicAmt || s.amount || 0) - (s.discountAmt || s.discount || 0)).toFixed(2);
    ...
    return `
      <tr style="text-align: center;">
        ...
        <td style="border: 1px solid #000; padding: 4px 8px 12px 8px; text-align: left;">${consignee}</td>
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${city}</td>
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${state}</td>
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${pincode}</td>
        ...
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${basicAmt}</td>
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${discount}</td>
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${taxable}</td>
      </tr>
    `;
  }).join("");
  ...
  return `
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: Arial, sans-serif; font-size: 10px; background: #f9fafb; padding: 10px; }
      </style>
    </head>
    <body>
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
        <thead>
          <tr style="background: #e5e7eb;">
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">AWB</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Date</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Consignee</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">City</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">State</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Pincode</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Product</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Weight</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Basic</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Discount</th>
            <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">Taxable</th>
          </tr>
        </thead>
        <tbody>
          ${rows}
...
// Generate only the content for shipment details (without full HTML structure)
...
async function generatePDFBuffer(invoiceData, qrCodeDataUrl) {
  let browser;
  ...
  try {
    ...
    // Generate combined HTML with both pages
    let combinedHTML = generateInvoiceHTML(invoiceData, qrCodeDataUrl);
    
    // If shipments exist, append shipment details page
    if (invoiceData.shipments && invoiceData.shipments.length > 0) {
      const shipmentDetailsContent = generateShipmentDetailsContent(invoiceData.shipments);
      
      // Insert shipment details before closing body tag with page break
...
async function uploadPDFToCloudinary(pdfBuffer, invoiceNumber) {
  return new Promise((resolve, reject) => {
    const uploadStream = cloudinary.uploader.upload_stream(
      {
        resource_type: "raw",
        folder: "invoices",
        public_id: `invoice_${invoiceNumber.replace(/\//g, "_")}_${Date.now()}`,
        format: "pdf",
      },
      (error, result) => {
...
export async function POST(req) {
  try {
    ...
    const { selectedInvoices } = body;

    if (!selectedInvoices || selectedInvoices.length === 0) {
      return NextResponse.json(
        { success: false, message: "No invoices selected" },
        { status: 400 }
      );
    }
    ...
    for (const selectedInvoice of selectedInvoices) {
      ...
      try {
        console.log(`📧 Processing invoice: ${selectedInvoice.invoiceNo}`);
        ...
        const customer = await CustomerAccount.findOne({
          accountCode: invoice.customer.accountCode,
        });
        ...
        if (invoice.qrCodeData?.[0]?.qrCode) {
          qrCodeDataUrl = await QRCode.toDataURL(invoice.qrCodeData[0].qrCode, {
            width: 200,
            margin: 1,
        ...
        const pdfBuffer = await generatePDFBuffer(invoice, qrCodeDataUrl);
        console.log("✅ PDF generated successfully");

        // Upload PDF to Cloudinary
        console.log("☁️ Uploading PDF to Cloudinary...");
        const cloudinaryResult = await uploadPDFToCloudinary(pdfBuffer, invoice.invoiceNumber);
          ...
          html: `
            ...
            <head>
              ...
              <style>
                ...
                .invoice-details { background-color: #f9f9f9; border-left: 4px solid #EA1B40; padding: 20px; margin: 20px 0; border-radius: 3px; }
                .invoice-details p { margin: 10px 0; }
                .invoice-details strong { color: #EA1B40; }
                ...
                .download-btn { display: inline-block; background-color: #EA1B40; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; font-weight: bold; margin: 20px 0; transition: background-color 0.3s; }
                .download-btn:hover { background-color: #c41635; }
            ...
            <body>
              <div class="container">
                ...
                <div class="content">
                  ...
                  <p>We hope this email finds you well. Please find attached your tax invoice for the period.</p>
                  ...
                  <div class="invoice-details">
                    ...
                    <p><strong>Total Shipments:</strong> ${invoice.shipments?.length || 0} AWBs</p>
                  </div>
                  
                  <div class="grand-total">
                    Grand Total: ₹${invoice.invoiceSummary.grandTotal.toFixed(2)}
                  </div>
                  
                  <p style="margin-top: 30px;">If you have any questions or concerns regarding this invoice, please don't hesitate to contact us.</p>
        ...
        console.error(`❌ Error processing invoice ${selectedInvoice.invoiceNo}:`, error);
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully sent ${results.length} invoice(s)`,
      results,
...
// New GET endpoint for fetching invoices
export async function GET(req) {
  try {
    ...
    const invoices = await Invoice.find(query)
      .select("invoiceNumber invoiceDate customer branch invoiceSummary")
      .sort({ invoiceDate: -1 })
      .lean();
    ...
    return NextResponse.json({
      ...
      invoices: invoices.map(inv => ({
        ...
        customerCode: inv.customer?.accountCode,

...
/**
 * Check if a rate is currently valid based on from/to dates
 * @param {Object} rate - Rate object with from and to dates
 * @param {Date} currentDate - Current date to check against (defaults to today)
 * @returns {boolean} - True if rate is currently valid
 */
function isRateCurrentlyValid(rate, currentDate = new Date()) {
  // If no dates are set, consider the rate as always valid
  ...
  return true; // Current date is within the valid range
}
 ...
 * 3. ?customer=Company Name (CG001)&sector=US - Returns filtered rates by sector (case-insensitive)
 * 
 * NEW: Automatically filters out expired rates based on from/to dates
 */
export async function GET(req) {
  try {
    let accountCode = req.nextUrl.searchParams.get("accountCode");
    ...
    // ✅ NEW: Filter by date validity FIRST (before sector filtering)
    const currentDate = new Date();
    const beforeDateFilter = rates.length;
    rates = rates.filter(rate => isRateCurrentlyValid(rate, currentDate));
    console.log(`📅 Date filtering: ${beforeDateFilter} → ${rates.length} rates (Current date: ${currentDate.toISOString().split('T')[0]})`);
    ...
    console.log(`📤 Returning ${transformedRates.length} transformed rates (date-filtered & active only)`);
    return NextResponse.json(transformedRates, { status: 200 });
  } catch (err) {
    console.error("❌ GET /shipper-tariff error:", err);
    return NextResponse.json(

...
// POST - Fetch debit notes based on multiple invoice numbers
export async function POST(request) {
  try {
    ...
    // Fetch debit notes matching the invoice numbers
    const debitNotes = await DebitNote.find(query)
      .sort({ "clientDetails.invoiceNo": 1 })
      .lean();

    console.log(`Found ${debitNotes.length} debit notes`); // Debug log
    ...
    console.error("Error in Debit Multiple Invoice POST request:", error);
    return NextResponse.json(
      { 
        success: false,

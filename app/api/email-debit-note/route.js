...
// Generate HTML for Debit Note Summary (Page 1)
function generateDebitNoteHTML(debitNoteData, customer, shipments) {
  const {
    clientDetails = {},
    amountDetails = {},
    debitItems = [],
  ...
  return `
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .debit-note-container { width: 210mm; min-height: 297mm; padding: 24px; background: #f9fafb; }
    ...
    <body>
      <div class="debit-note-container">
        ...
        <!-- Debit Note Summary Table -->
        <h2 style="text-align: center; font-weight: bold; margin: 16px 0;">Debit Note Summary</h2>
        ...
        <div class="terms-section">
          <div class="terms">
            ...
            <ul style="margin-top: 8px;">
              <li><strong>1.</strong> On receipt of the debit note the adjustment should be made within 24 hours.</li>
              ...
              <li><strong>4.</strong> This is a computer generated debit note and it does not require signature.</li>
            </ul>
          </div>
...
function generateShipmentDetailsContent(shipments) {
  ...
  const rows = shipments.map((s) => {
    ...
    const debitAmount = Number(s.debitAmount || 0).toFixed(2);
    ...
    return `
      <tr style="text-align: center;">
        ...
        <td style="border: 1px solid #000; padding: 4px 4px 12px 4px;">${debitAmount}</td>
      </tr>
    `;
  }).join("");
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
      <thead>
        <tr style="background: #e5e7eb;">
          ...
          <th style="border: 1px solid #000; padding: 4px 4px 12px 4px;">DR Amount</th>
        </tr>
      </thead>
      <tbody>
        ${rows}
...
async function generatePDFBuffer(debitNoteData, customer, shipments) {
  let browser;
  ...
  try {
    ...
    let combinedHTML = generateDebitNoteHTML(debitNoteData, customer, shipments);
...
async function uploadPDFToCloudinary(pdfBuffer, debitNoteNumber) {
  return new Promise((resolve, reject) => {
    const uploadStream = cloudinary.uploader.upload_stream(
      {
        resource_type: "raw",
        folder: "debit-notes",
        public_id: `debit_note_${debitNoteNumber.replace(/\//g, "_")}_${Date.now()}`,
        format: "pdf",
      },
      (error, result) => {
...
export async function POST(req) {
  try {
    ...
    const { selectedDebitNotes } = body;

    if (!selectedDebitNotes || selectedDebitNotes.length === 0) {
      return NextResponse.json(
        { success: false, message: "No debit notes selected" },
        { status: 400 }
      );
    }
    ...
    for (const selectedDebitNote of selectedDebitNotes) {
      let cloudinaryPublicId = null;
      
      try {
        console.log(`📧 Processing debit note: ${selectedDebitNote.debitNoteNo}`);

        // Fetch full debit note data
        const debitNote = await DebitNote.findOne({
          "clientDetails.invoiceNo": selectedDebitNote.debitNoteNo,
        });
        ...
        // Fetch shipment details for all AWBs in debit items
        const awbNumbers = debitNote.debitItems.map(item => item.awbNo);
        const shipments = await Shipment.find({
          awbNo: { $in: awbNumbers }
        }).lean();

        // Add debit amounts to shipments
        const shipmentsWithDebit = shipments.map(shipment => {
          const debitItem = debitNote.debitItems.find(item => item.awbNo === shipment.awbNo);
          return {
            ...shipment,
            debitAmount: debitItem?.debitAmount || 0
          };
        });

        console.log(`✅ Found ${shipmentsWithDebit.length} shipments`);

        // Generate PDF
        console.log("📄 Generating PDF...");
        const pdfBuffer = await generatePDFBuffer(debitNote, customer, shipmentsWithDebit);
        console.log("✅ PDF generated successfully");

        // Upload PDF to Cloudinary
        console.log("☁️ Uploading PDF to Cloudinary...");
        const cloudinaryResult = await uploadPDFToCloudinary(pdfBuffer, debitNote.clientDetails.invoiceNo);
        ...
        const mailOptions = {
          ...
          subject: `Debit Note - ${debitNote.clientDetails.invoiceNo}`,
          html: `
            <!DOCTYPE html>
            <html>
            <head>
              ...
              <style>
                ...
                .debit-note-details { background-color: #f9f9f9; border-left: 4px solid #EA1B40; padding: 20px; margin: 20px 0; border-radius: 3px; }
                .debit-note-details p { margin: 10px 0; }
                .debit-note-details strong { color: #EA1B40; }
            ...
            <body>
              <div class="container">
                ...
                <div class="content">
                  ...
                  <p>We hope this email finds you well. Please find attached your debit note for adjustment.</p>
                  
                  <div class="debit-note-details">
                    <p><strong>Debit Note Number:</strong> ${debitNote.clientDetails.invoiceNo}</p>
                    <p><strong>Debit Note Date:</strong> ${new Date(debitNote.clientDetails.invoiceDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                    <p><strong>Total Shipments:</strong> ${debitNote.debitItems?.length || 0} AWBs</p>
                  </div>
                  
                  <div class="grand-total">
                    Debit Amount: ₹${debitNote.amountDetails.grandTotal.toFixed(2)}
                  </div>
                  
                  <p style="margin-top: 30px;">If you have any questions or concerns regarding this debit note, please don't hesitate to contact us.</p>
          ...
          attachments: [
            {
              filename: `DebitNote_${debitNote.clientDetails.invoiceNo.replace(/\//g, "_")}.pdf`,
              content: pdfBuffer,
        ...
        results.push({
          debitNoteNo: debitNote.clientDetails.invoiceNo,
          email: customer.billingEmailId,
          status: "sent",
        });
      } catch (error) {
        console.error(`❌ Error processing debit note ${selectedDebitNote.debitNoteNo}:`, error);
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully sent ${results.length} debit note(s)`,
      results,
      errors: errors.length > 0 ? errors : undefined,
    });
  } catch (error) {
    console.error("❌ Email debit note error:", error);
    return NextResponse.json(
...
// GET endpoint for fetching debit notes
export async function GET(req) {
  try {
    ...
    const debitNotes = await DebitNote.find(query)
      .select("clientDetails amountDetails debitItems")
      .sort({ "clientDetails.invoiceDate": -1 })
      .lean();

    // Fetch shipment details for weight calculation
    const debitNotesWithWeight = await Promise.all(
      debitNotes.map(async (dn) => {
        const awbNumbers = dn.debitItems.map(item => item.awbNo);
        const shipments = await Shipment.find({
          awbNo: { $in: awbNumbers }
        ...
        return {
          debitNoteNo: dn.clientDetails.invoiceNo,
          debitNoteDate: dn.clientDetails.invoiceDate,
          customerCode: dn.clientDetails.accountCode,
          customerName: dn.clientDetails.customerName,
          branch: dn.clientDetails.branch,
          grandTotal: dn.amountDetails?.grandTotal || 0,
          weight: totalWeight,
        };
      })
    );
    ...
    return NextResponse.json({
      success: true,
      debitNotes: debitNotesWithWeight,
    });
  } catch (error) {
